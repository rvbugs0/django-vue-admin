import { request } from '@/api/service'

export const crudOptions = (vm) => {
   return {
     pageOptions: {
       compact: true
     },
     options: {
       tableType: 'vxe-table',
       rowKey: false, // must be set, true or false
       height: '100%',
       rowId: 'code',
       highlightCurrentRow: true,
       treeConfig: { // tree data configuration
         lazy: true,
         children: 'children',
         hasChild: 'hasChildren',
         loadMethod: ({ row }) => {
           return request({
             url: '/api/system/area/',
             method: 'get',
             params: { pcode: row.code, limit: 999 }
           }).then(ret => {
             ret.data.data.map(value => { value.hasChildren = value.pcode_count !== 0 })
             row. hasChildren = false
             return ret.data.data
           })
         },
         iconLoaded: 'el-icon-loading' // Beautify the loading icon
       }
     },
     rowHandle: {
       show: false,
       width: 140,
       view: {
         thin: true,
         text: '',
         show: false,
         disabled () {
           return !vm.hasPermissions('Retrieve')
         }
       },
       edit: {
         thin: true,
         text: '',
         show: false,
         disabled () {
           return !vm.hasPermissions('Update')
         }
       },
       remove: {
         thin: true,
         text: '',
         show: false,
         disabled () {
           return !vm.hasPermissions('Delete')
         }
       }
     },
     viewOptions: {
       componentType: 'form'
     },
     formOptions: {
       defaultSpan: 24, // default form span
       width: '30%'
     },
     indexRow: { // Or pass true directly, do not display title, do not center
       title: 'S.No.',
       align: 'center',
       width: 100
     },
     columns: [
       {
         title: 'ID',
         key: 'id',
         width: 90,
         disabled: true,
         form: {
           disabled: true
         }
       },
       {
         title: 'Parent region',
         key: 'pcode',
         show: false,
         search: {
           disabled: false
         },
         type: 'area-selector',
         // dict: {
         // url: areaJointPrefix,
         // lazy: true,
         // isTree: true,
         // cache: false,
         // value: 'code', // attribute name of the value field in the data dictionary
         // label: 'name', // attribute name of the label field in the data dictionary
         // children: 'children' // attribute name of the children field in the data dictionary
         // },
         form: {
           component: {
             showAllLevels: false, // only show the last level
             props: {
               elProps: {
                 clearable: true,
                 showAllLevels: false, // only show the last level
                 props: {
                   checkStrictly: true, // You don't need to select the last level
                   emitPath: false,
                   clearable: true
                 }
               }
             }
           }
         }
       },
       {
         title: 'Title',
         key: 'name',
         search: {
           disabled: false
         },
         treeNode: true,
         width: 160,
         type: 'input',
         form: {
           rules: [ // form validation rules
             { required: true, message: 'Name is required' }
           ],
           component: {
             placeholder: 'Please enter a name'
           },
           itemProps: {
             class: { yxtInput: true }
           }
         }
       },
       {
         title: 'Area Code',
         key: 'code',
         search: {
           disabled: false
         },
         type: 'input',
         form: {
           rules: [ // form validation rules
             { required: true, message: 'Area code is required' }
           ],
           component: {
             placeholder: 'Please enter the area code'
           },
           itemProps: {
             class: { yxtInput: true }
           }
         }
       },
       {
         title: 'Pinyin',
         key: 'pinyin',
         search: {
           disabled: true
         },
         type: 'input',
         form: {
           rules: [ // form validation rules
             { required: true, message: 'Pinyin is required' }
           ],
           itemProps: {
             class: { yxtInput: true }
           },
           component: {
             placeholder: 'Please enter pinyin'
           }
         }
       }, {
         title: 'Regional level',
         key: 'level',
         search: {
           disabled: true
         },
         type: 'input',
         form: {
           disabled: false,
           rules: [ // form validation rules
             { required: true, message: 'Pinyin is required' }
           ],
           itemProps: {
             class: { yxtInput: true }
           },
           component: {
             placeholder: 'Please enter pinyin'
           }
         }
       }, {
         title: 'Initial letter',
         key: 'initials',
         form: {
           rules: [ // form validation rules
             { required: true, message: 'The first letter is required' }
           ],
           itemProps: {

            class: { yxtInput: true }
          },
          component: {
            placeholder: 'Please enter the first letter'
          }
        }
      },
      {
        title: 'Whether to enable',
        key: 'enable',
        search: {
          disabled: false
        },
        width: 90,
        type: 'radio',
        dict: {
          data: vm.dictionary('button_status_bool')
        },
        form: {
          value: true,
          itemProps: {
            class: { yxtInput: true }
          }
        }
      }
    ].concat(vm.commonEndColumns())
  }
}